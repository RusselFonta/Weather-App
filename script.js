...
// Display health risk advice based on humidity and temperature
function environmentalRange (temps, hums) {
  ...
  if (temps > 35 && hums > 60) {
  ...
  } else if (temps >= 25 && temps <= 35 && hums > 50 && hums <= 70) {
    title = 'Warm & Muggy'
    Effect = 'Hard to breathe, can trigger asthma attacks and allergies'
    ...
    Effect = 'Increased risk of indoor mold, damp feeling, heavy shivering'
...
async function getWeatherInfo () {
  ...
  try {
    ...
    const localSunsetMs = (response.sys.sunset + response.timezone) * 1000
    const sunsetDate = new Date(localSunsetMs)
    ...
    dateFormat(targetDate, foreResponse)
...
function dateFormat (targetDate, foreResponse) {
  const days = [
    'Sunday',
    'Monday',
    'Tuesday',
  ...
  if (actualTime) {
    actualTime.textContent = `${days[targetDate.getUTCDay()]} ${targetDate.getUTCDate()} ${month[targetDate.getUTCMonth()]}`
  }

  const dailyMap = new Map()

  foreResponse.list.forEach((item) => {
    const itemLocalMs = (item.dt + foreResponse.city.timezone) * 1000
    const itemLocalDate = new Date(itemLocalMs)
    const dateKey = itemLocalDate.toISOString().split('T')[0]

    if (!dailyMap.has(dateKey)) {
      dailyMap.set(dateKey, item)
    } else {
      const currentChoice = dailyMap.get(dateKey)
      const currentHour = new Date(
        (currentChoice.dt + foreResponse.city.timezone) * 1000
      ).getUTCHours()
      const newHour = itemLocalDate.getUTCHours()
...
document.addEventListener('DOMContentLoaded', () => {
  const savedCity = localStorage.getItem('LastSearchCity')
  if (savedCity) {
