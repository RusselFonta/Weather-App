...
async function getWeatherInfo() {
  ...
  try {
    ...
    // const {latitude , longitude} = response.coord
    // const uvLightUrl = `https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&units=metric&appid=${apiKey}`
    // const unit = await fetch(uvLightUrl)
    // if(!unit.ok){
    //   throw new Error(` Response status ${unit.status}`)
    // }
    // const unitResponse = unit.json()
